...
Many of these problems are improved upon in the recent CHM13 assembly, the first complete sequence of a human genome \cite{nurk_complete_2022}.
...
Adam Novak and I are co-first authors.
...
The decomposition of the graph into nested snarls and chains is described by a snarl tree (Figure \ref{fig:algorithm_overview} A). 

\subsection{Giraffe algorithm overview}

The long read Giraffe algorithm follows the common seed-chain-extend framework \citep{sahlin_survey_2023} for mapping algorithms (Figure \ref{fig:algorithm_overview}).
As in the original short read algorithm, long read Giraffe uses a minimizer index of the haplotypes of the GBWT to find seed alignments (Figure \ref{fig:algorithm_overview} B).
...
The seeds are then chained together (Figure \ref{fig:algorithm_overview}D, Section \ref{sec:sup-chaining}).
...
To efficiently calculate reachability and distance in a graph, we use a novel data structure called a \emph{zip code tree} that represents distances among a set of seeds (Figure \ref{fig:algorithm_overview} C).
...
Base-level alignment is done between consecutive seeds in the chain and out from the first and last seeds (Figure \ref{fig:algorithm_overview}E, Section \ref{sec:sup-aligning}).
...
\section{Preface}
This section contains the supplement of long read Giraffe preprint in Chapter \ref{chapter:lr-giraffe}).
...
Each node in the zip code tree represents the position of a seed in the graph or the boundary of a snarl or chain containing a seed (Figure \ref{fig:algorithm_overview}C).
...
The distances between seeds can be found by walking through the zip code tree (Figure \ref{fig:algorithm_overview}D).
...
These jobs were run on a machine with some specs that I need to find.
