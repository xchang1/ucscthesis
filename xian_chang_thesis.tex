...
Lexicographical order is often used to select minimizers, but the selection $f$ can be optimized to improve the density of minimizers in the reference \cite{zheng_minimizers_2020,marcais_minimizers_2018}.
...
A simple example of a text index is a suffix trie, which stores all suffixes of a template string in a tree structure where edges are labelled with characters, such that each suffix is represented in a path from the root of the tree down to a leaf \cite{li_survey_2010}.
Because any substring of the template string must be a prefix of one of its suffixes, exact substring queries can be done in a walk down from the root of the suffix trie.
...
The $r$-index makes it feasible to index databases of whole genomes.
...
Unlike $k$-mer based methods, text indexes produce variable-length seeds that may be maximal exact matches or maximal unique matches.
...
For mapping long reads, an unordered cluster of seeds was often insufficient for 
