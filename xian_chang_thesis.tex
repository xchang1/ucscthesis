...
First, I present an index for finding minimum distances in pangenomes and use it to develop a clustering algorithm.
I then describe a read mapping tool for short reads that incorporates the distance index and clustering algorithm.
Finally, I describe extensions to the short read mapper for mapping long reads.
...
Next, I describe a short read-to-pangenome mapper (Chapter~\ref{chapter:sr-giraffe}) and a long read-to-pangenome mapper (Chapter ~\ref{chapter:lr-giraffe}), and show how each improves variant calling relative to standard linear pipelines.

\chapter{Background}
\label{chapter:background}

\section{Linear reference genomes and standard genomic analyses}
\label{sec:background:linear-genomes}

%The general goal of any genomic analysis is to understand the contents of a genome.
%The scope of such an analysis can vary widely: from identifying a single single nucleotide variant (SNV) on a single gene, to finding the entire sequence of an individual's genome, to comparing the genetic makeup of populations. 
The human reference genome is ingrained in nearly all modern genomic analyses.
Resequencing experiments drive much of 
Nearly all genomic analyses rely on a reference genome to provide a common language for describing and interpreting new samples.
A reference genome is a high-quality, annotated genome assembly that is taken to be representative of a species.
New samples are described relative to the reference genome in terms of variants where the sample's sequence differs from the reference, and the annotations from the reference are used to understand the functional impact of a sample's variants.

The reference genome is frequently used to perform analyses as well.
In 
...
Recently, the first complete sequence of a human genome was found \cite{nurk_complete_2022}.
Unlike the GRCh38 assembly, the CHM13 assembly is the sequence of a single individual (a haploid hydatidiform mole) and includes the 8\% of the genome that remained unsequenced in GRCh38, opening these region to analyses for the first time \cite{nurk_complete_2022}.
...
%TODO Talk about chm13 

Using a linear genome as a reference for analyzing new samples introduces a bias causing the new sample to appear more similar to the reference \cite{noauthor_computational_2016,eizenga_pangenome_2020}.
...
Most human genetic variation occurs across populations, rather than within populations \cite{nih_understanding_2007}, meaning that the majority of the genome will still be subject to the same reference bias as would stem from any randomly selected reference genome. 
...
Pangenomes are collections of genomic sequences from a population.
Because they can represent multiple sequences at each locus, they can better represent variations in a population and have been shown to reduce reference bias relative to using a linear genome \cite{sherman_pan-genomics_2020,noauthor_computational_2016}.
Due to recent advances in genome sequencing and assembly, there has been a proliferation of large-scale sequencing projects producing large numbers of high-quality genomes and catalogues of variations from diverse populations\cite{1000gp_2015,audano_hgsvc,hapmap_consortium_second_2007}.
These advances have made it increasingly feasible to produce reference pangenomes containing variations from thousands of reference genomes, including variants in regions of the genome that were previously uncharacterized.
However, while the data to create a pangenome is becoming available, efficiently using a pangenome reference for genomic analysis requires data structures and algorithms that can deal with large amounts of sequence.
...
It also includes portions of the supplement that describe or are pertinent to my contributions.
The remainder of the supplement referenced by the main text can be found in the Appendix, Section \ref{sec:appendix_sr_giraffe}.
...
With modern sequencing technologies and understanding of population genetics, it is now possible to generate reference-quality genome assemblies at the scale necessary for creating a representative human reference pangenome.

The first reference pangenomes are just beginning to be produced, both as global and population-specific references.
With the growth of pangenomics, both in the pangenomes themselves and the popularity of the field, there is a corresponding need for bioinformatics tools capable of scaling to large pangenomes and datasets. 
...
Although we have shown Giraffe to be fast and accurate for the current HPRC graphs, there is still work to be done to ensure that it works with increasingly complicated graphs.
...
This has been a recent area of exploration in the lab.
One potential solution that I discussed with Simon Heumos is to use his layout algorithm from odgi to estimate a 2-dimensional embedding of the netgraph.
...
The embedding could be paired with a bit-vector representing reachability between nodes, but even a bit vector with quadratic cost may be too big. 
...
Even when distance indexing is solved, the problem remains that as pangenomes grow in size, analyses using them will inevitably become slower and at times less accurate.
Counterintuitively, adding more variants to a pangenome can actually impede read mapping; when there are too many targets for mapping, reads may erroneously map to a rare variant that happens to be more similar to the read than the correct alignment. %TODO cite forge
The solution used in the Giraffe papers is to use a frequency-filtered graph that has low-frequency variants removed~\cite{}, but this solution is not ideal as it throws away potentially valuable information.
Another solution for mapping reads from a single individual is to sample the graph to find a subgraph that best represents the individual: a so-called personalized pangenome \cite{}.
This strategy has been shown to be faster and produce better variant calling results than using the frequency filtered graph.

The success of personalized pangenomes demonstrates the versatility of a pangenome reference.
Pangenomes could be customized for a particular individual or application, by either adding or removing disease-specific, population-specific, or individual-specific variants.
%TODO: idk if I can include this?
The Emblask-Weaver pipeline for example adds population-specific small variants to a graph containing structural variants.
A similar strategy could be imagined for Giraffe for mapping to graphs with additional variants added.
In general, mapping to a modified graph with Giraffe would be slow because the graph must be re-indexed to accommodate the new variants. 
The personalized pangenome workflow is efficient because it reduces the pangenome to just two haplotypes, which can be indexed quickly.
Adding variants to a pangenome, particularly adding large chromosomal rearrangements, complicates the topology of the graph and the indexes, particularly the snarl decomposition. 
...
Rather than changing the graph itself and re-indexing, the variants would only be considered during mapping.
...
Read mapping is one of the first necessities of pangenome workflows, but there remain many challenges in the way of the general adoption of a pangenome standard.
To convince researchers and clinicians who still haven't adopted the GRCh38 standard to switch to a pangenome, pangenomics must be clearly better and simple and easy to use.



-To make pangenomes standard, we need to convince people to use them and make them as easy as possible to use
-Deal with bigger graphs
-Haplotype sampling to personalize pangenomes
-application/population specific pangenomes by adding or removing variants
-Questions about how to best represent a pangenome- what do we consider homology, how much variation should we include, balance utility with representation
-inevitable loss of information when using reference downstream-represent variants relative to the reference genome
-coordinate system




As more and more high-quality data is being produced, bioinformatics tools must keep pace with methods to efficiently process and analyze  

They are capable of more accurately representing the genetic diversity of a population and can reduce biases in genomic analyses.
...
I plan to develop pangenome methods for common genomic tasks.
I will create a distance index for efficiently calculating minimum distances in pangenome graphs.
Using algorithms developed against the distance index, I will contribute to a read-to-graph mapper for mapping short and long reads to pangenome graphs.
Finally, I will write a graph-to-graph mapper for comparing pangenome graphs.
Each of these aims will contribute to establishing a robust ecosystem of tools to support a more complete standard reference genome.
