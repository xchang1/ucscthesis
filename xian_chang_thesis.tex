...
Pangenomes are commonly represented using a graph format where nodes represent nucleotide sequences and edges occur between sequences that are adjacent in the genome sequences of the pangenome.
...
The current reference genome, GRCh38, has build upon the initial assembly by closing gaps, adding alternate loci, improving annotations, and generally improving the quality of the sequence \cite{schneider_evaluation_2017}.
...
Many of these problems are improved upon by the recent CHM13 assembly, which is the first complete sequence of a human genome \cite{nurk_complete_2022}.
...
Recent advancements in genome sequencing technologies, genome assembly algorithms, and a greater understanding of human population genetics has brought this prospect closer to a reality.
...
Currently, the two predominant long read sequencing technologies are single-molecule real-time (SMRT) sequencing and nanopore sequencing \cite{lr_review_23}.
...
Sequencing technologies can also capture information from the genome beyond the nucleotide sequence.
...
The first draft of the human genome was published in 2001, eleven years after the initial launch of the project, and cost about three billion dollars \cite{miga_need_2021}.
Since then, the cost of genome assembly has dropped dramatically, largely due to new sequencing technologies and the algorithms that assemble reads into a genome sequence \cite{miga_envisioning_2023}.
Modern assembly algorithms use long reads to resolve the most repetitive and complex regions of the genome \cite{cheng_hifiasm_2021,rautiainen_verkko_2023,shafin_shasta_2020}.
Haplotype phasing, the problem of distinguishing between the maternal and paternal haplotype, can be done using chromosome conformation capture reads, ultra-long nanopore reads, or sequencing date from the parents \cite{cheng_hifiasm_2021}.
A high quality, phased genome now costs between ten and fifteen thousand dollars \cite{miga_envisioning_2023}.
The first published draft of a human pangenome reference produced assemblies with an average quality value of 53.57 \cite{hprc_pangenome_2023}.
...
This greater view of human genetic diversity both highlights the shortcomings of the human reference genome and provides guidance for how to create a reference that better represents all of humanity \cite{ballouz_is_2019,miga_need_2021}.
...
With recent advances in genome sequencing and assembly algorithms, it is now possible to produce reference-quality and even telomere-to-telomere genome sequences with ever-decreasing time and cost \cite{miga_envisioning_2023}.
...
The initial draft of the HPRC's human reference pangenome contained the existing GRCh38 and CHM13 reference genomes as well as high-quality, phased, and annotated genomes of 47 individuals selected from existing 1000 Genomes Project cell lines and prioritized for their coverage of genetic variants and biogeographic origin \cite{hprc_pangenome_2023}.
...
In addition to this global pangenome project, here have been several recent efforts to establish ancestry-specific pangenomes for underrepresented populations \cite{pacific_pangenome_2024,chinese_pangenome_2023,arab_pangenome_2024}.
...
The \texttt{vg} toolkit originally supported two formats for representing variation graphs: the original VG format and XG format \cite{garrison_variation_2018}.
...
The problem of read mapping is to take sequencing reads from a sample and find a placement of the reads in a reference.
...
This can be used for finding exact matches between a read and a reference sequence.
...
The Run-Length FM-Index (RLFM-index) can index multiple genomes, represented as a single concatenated sequence, with a memory requirement linear in the number of runs of the same character in the BWT \cite{makinen_succinct_2005,makinen_storage_2010}.
A compression scheme dependent on the repetitiveness of the template sequence is very effective for genome sequences, but using this index to locate substrings requires an additional data structure that increases memory requirement.
...
Others mappers use generalizations of the BWT for indexing graphs \cite{garrison_variation_2018,kim_hisat2_2019,siren_indexing_2014,siren_indexes_2020} or collections of genomes \cite{rossi_moni_2022}.
...
One such example is partial order alignment (POA), an algorithm for constructing and aligning to multiple sequence alignments \cite{lee_multiple_2002}.
...
Other mappers align to cyclic graphs by converting them to DAGs before using an acyclic aligner \cite{garrison_variation_2018} or use graph extensions of linear alignment algorithms \cite{rautiainen_bit-parallel_2019,zhang_fast_2022}. 
...
DeepVariant is convolutional neural network-based variant caller for calling small variants \cite{poplin_universal_2018}. 
...
Calling structural variants is a more challenging problem, particularly around highly repetitive regions.
